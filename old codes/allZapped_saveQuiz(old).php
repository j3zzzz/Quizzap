    ...
    try {
        ...
        for ($i = 0; $i < count($_POST['questions']); $i++) {
            ...
            switch ($question_type) {
                ...
                case 'enumeration':    
                    // Store correct answers and track the total number of expected items
                    if (isset($_POST['correct_option'][$i]) && is_array($_POST['correct_option'][$i])) {
                        $enumeration_answers = $_POST['correct_option'][$i];
                        $expected_count = count($enumeration_answers);

                        // Update the question with the expected enumeration count
                        $stmt = $conn->prepare("UPDATE questions SET expected_enumeration_count = ? WHERE question_id = ?");
                        $stmt->bind_param("ii", $expected_count, $question_id);
                        if (!$stmt->execute()) {
                            throw new Exception("Error updating enumeration question details: " . $stmt->error);
                        }

                        // Store each enumeration answer
                        foreach ($enumeration_answers as $answer_text) {
                            if (!empty($answer_text)) {
                                $stmt = $conn->prepare("INSERT INTO answers (question_id, answer_text, is_correct) VALUES (?, ?, 1)");
                                $stmt->bind_param("is", $question_id, $answer_text);
