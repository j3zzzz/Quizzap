...
foreach ($answers as $question_id => $answer) {
    ...
    if ($question_type === 'true_or_false' || $question_type === 'multiple_choice' || $question_type === 'drag_and_drop') {
        ...
        $sql = "SELECT a.is_correct, a.answer_text 
                FROM answers a 
                INNER JOIN questions q ON a.question_id = q.question_id
                WHERE a.answer_id = ? AND a.question_id = ?";
        ...
        $stmt->bind_param("ii", $answer, $question_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if ($row['is_correct'] == 1) {
                ...
                $wrong_answers[$question_id] = [
                    'submitted_id' => $answer,
                    'submitted_text' => $row['answer_text']
                ];

                $sql_correct = "SELECT answer_text FROM answers WHERE question_id = ? AND is_correct = 1";
                ...
                if ($result_correct->num_rows > 0) {
                    $correct_row = $result_correct->fetch_assoc();
                    $wrong_answers[$question_id]['correct_answer'] = $correct_row['answer_text'];
                }
                $stmt_correct->close();
            }
        } else {
            echo json_encode([
                "success" => false, 
                "error" => "Answer not found for question ID." . $question_id . "and answer ID: " . $answer
            ]);
            exit;
        }
        $stmt->close();
