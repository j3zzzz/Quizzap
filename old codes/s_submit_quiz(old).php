...
function logError($message) {
    $logFile = 'quiz_submission_error.log';
...
$stmt = $conn->prepare($sql);
if (!$stmt) {
    logError("Failed to prepare quiz type statement: " . $conn->error);
    echo json_encode(["success" => false, "error" => "Failed to prepare statement: " . $conn->error]);
    exit;
}
...
if ($result->num_rows > 0) {
    ...
    logError("Quiz Type: " . $quiz_type);
} else {
    logError("Quiz not found for ID: $quiz_id");
...
foreach ($answers as $question_id => $answer) {
    ...
    if ($quiz_type === 'True or False' || $quiz_type === 'Multiple Choice' || $quiz_type === 'Drag & Drop') {
        ...
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            logError("Failed to prepare answer statement: " . $conn->error);
            echo json_encode(["success" => false, "error" => "Failed to prepare statement: " . $conn->error]);
            exit;
        }
        ...
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            logError("Correct Answer for Question $question_id: " . $row['answer_text']);
            logError("Submitted Answer: " . $answer);
            //trim and convert yung correct answers sa lowercase for case-insensitive comparison
            $correct_answers = array_map(function($answer) {
                return trim(strtolower($answer));
            }, explode(',', $row['answer_text']));
            
            //i-submit na yung answers na naka trim at converted na sa lowercase
            $submitted_answers = array_map(function($answer) {
                return trim(strtolower($answer));
            }, explode(',', $answer));

            $is_correct = (count($submitted_answers) === count($correct_answers) && count(array_diff($submitted_answers, $correct_answers)) === 0);

            if ($is_correct) {
                $score += $correct_count($correct_answers);
                logError("Question $question_id is CORRECT");
            } else {
                logError("No answer found for Question $question_id");
                $wrong_answers[$question_id] = $answer;
                logError("Question $question_id is INCORRECT");
            }
        } else {
...
if ($is_final_submit) {
   ...
   // Log final results
   logError("Quiz Submission Successful - Score: $score / $total");
