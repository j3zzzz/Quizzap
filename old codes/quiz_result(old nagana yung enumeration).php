...
// Fetch quiz type
$quiz_type_sql = "SELECT question_id, question_type FROM questions WHERE quiz_id = ?";
...
while ($row = $quiz_type_result->fetch_assoc()) {
    $question_id = $row['question_id'];
    $question_type = $row['question_type'];

    if ($question_type === 'enumeration') {
        // Count the correct answers for enumeration questions
        $answer_sql = "SELECT answer_text FROM answers WHERE question_id = ?";
        ...
        if ($answer_result && $answer_result->num_rows > 0) {
            ...
            $correct_answers = explode(',', $answer_row['answer_text']);
            $total_correct_answers += count($correct_answers);
        }
        $answer_stmt->close();
    } else {
        // Increment total for non-enumeration questions
...
<body>
    ...
    <div class="container">
    <p class="score">Your score: <?php echo $score . " / " . $total_correct_answers; ?></p>
